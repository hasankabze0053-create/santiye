...
function refactorLawAnalysisResultScreen() {
    ...
    // 3. Inject into ScannerOverlay safely
    ...
    // 4. Inject into RiskGauge safely
    ...
    code = code.replace(
        ...
        "borderColor: isDarkMode ? '#1e1e1e' : theme.borderLight"
    );
    code = code.replace(
    ...
    // Hero Card Gradient: colors={['#181818', '#111']} -> colors={isDarkMode ? ['#181818', '#111'] : [theme.surface, theme.surfaceSecondary]}
    code = code.replace(/colors=\{\['#181818', '#111'\]\}/g, "colors={isDarkMode ? ['#181818', '#111'] : [theme.surfaceSecondary, theme.surface]}");
    ...
    code = code.replace(/colors=\{\['#1a1a1a', '#111'\]\}/g, "colors={isDarkMode ? ['#1a1a1a', '#111'] : [theme.surfaceSecondary, theme.surface]}");
